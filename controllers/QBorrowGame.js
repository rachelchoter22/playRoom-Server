...
const functions = {
    ...
    BorrowGame: (req, res) => {
        try {
            let { customerId, gameId } = req.body;
            var sql = `insert into playroom.borrowedgame values(null,'${customerId}','${gameId}',0,
            current_date(),null);`;
            var sql2 = `update playroom.games set existingNumber=existingNumber-1 where id=${gameId};`
            con.query(sql, (err, result) => {
                if (err)
